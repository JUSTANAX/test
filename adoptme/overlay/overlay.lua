...
local COLORS = {
    ...
    -- Зелья - приглушённо-пурпурный. Второе число не должно спорить с
    -- оранжевым за внимание, но должно быть сразу отличимо: две одинаковые
    -- по цвету строки на иконке читались бы как одно длинное число.
    -- Светлый оттенок выбран не на глаз: ярлык лежит прямо на иконке
    -- питомца, а они бывают и тёмными, и почти белыми, поэтому тёмный
    -- пурпур утонул бы даже с обводкой.
    potion = Color3.fromHex("C89BFF"),
...
-- Пять знаков после запятой - не «на глаз», а предел самих данных: в
-- каталоге есть 125 цен вида 0.00375, и ни одной точнее. Меньше пяти -
-- потеря (0.00375 показалось бы как 0.0038), больше - выдумывание цифр.
local DECIMALS = 5
...
local function formatValue(v)
    ...
    return trimZeros(string.format("%." .. DECIMALS .. "f", v))
end

--- Цена в зельях езды, БЕЗ округления до сетки.
---
--- Сайт в своём режиме «Ride Pot» округляет до 0.05 зелья
--- (Math.round(154*v*20)/20), и сначала так делал и я. Но тогда числа на
--- иконках не складывались в итог: 0.85 + 1.4 давало 2.25, а в сумме
--- стояло 2.2. Босс попросил точные цены, и это заодно чинит расхождение -
--- без округления слагаемые сходятся с суммой копейка в копейку.
---
--- Формат тот же, пять знаков: множитель целый, поэтому произведение не
--- бывает точнее исходной цены. Заодно это гасит машинный шум - иначе
--- 154 * 3.7 показалось бы как 569.8000000000001.
local function formatPotions(v)
    if type(v) ~= "number" then
        return ""
    end
    return trimZeros(string.format("%." .. DECIMALS .. "f", RIDE_POTION_FACTOR * v))
end
